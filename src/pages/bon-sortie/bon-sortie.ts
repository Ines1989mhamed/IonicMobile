...
export class BonSortiePage implements OnInit {
  ngOnInit(): void {
    // this.getAllFiches();
 }
 //------------------------ pdf-------------
...
//----------------------------------
  ...
  Fiche: FicheSortie = new FicheSortie();
  options: BarcodeScannerOptions;
  //---------Constructeur---------------
  constructor(public navCtrl: NavController, public navParams: NavParams, 
    private dataService : DataService, public file: File,
...
makePdf() {
  ...
  let body = this.Fiche.LignesProduits.map(produit => {
    return [produit.Num_Ligne, produit.Produit.Designation, produit.Quantite];
});
body.unshift(['N°', 'Designation', 'Quantité']);
  ...
  content: [
      {
          ...
          {text:'Bon De Livraison ' , style: 'center',color : '#B22222'},
      ...
      },
      {
          columns: [
              ...
              { text: 'N° : BS ',style :'textSize',alignment:'center' },             
          ]
      },
      '\n','\n', 
      {
          columns: [
              { text: 'Client : ',style :'textSize'},
              { text: this.Fiche.Client.Nom,style : 'Size'},
             
              { text: 'Adr. livraison :  ',style :'textSize',alignment:'right'},
              { text: this.Fiche.Client.Adresse,style : 'Size'},
              { text: 'Evennement :  ',style :'textSize'},
              { text: this.Fiche.Evennement,style : 'Size'},
          ]
      },
      '\n', 
      {
        columns:[
          { text: "Transporteur : ",style:'textSize' },
          { text: this.Fiche.Transporteur.Nom,style:'Size'},
          { text: 'Telephone :  ',style :'textSize',alignment:'center'},
          { text: this.Fiche.Transporteur.Tel,style : 'Size'},
          { text: 'Voiture :  ',style :'textSize',alignment:'right'},
          { text: this.Fiche.Voiture.Matricule,style : 'Size'}
        ]
      },
      '\n', 
      {
        columns: [
          ...
          { text: this.Fiche.Employees,style:'Size'},
        ]
      },
      '\n', 
      {
        columns: [
          {text: 'Date début', style: 'textSize'}, 
          {text: this.Fiche.DateDeb, style:'Size',alignment:'center'},
          {text: 'Date fin', style: 'textSize'}, 
          {text: this.Fiche.DateFin, style: 'Size'}
        ]
      },'\n','\n',
      {
...
//      .then(dirEntry => {
//         this.file.getFile(dirEntry, 'Fiche.pdf', { create: true })
//           .then(fileEntry => {
...
//   });
  ...
  valider() {
    ...
    if (filteredLigneProduitIndex != -1) {
      ...
      ligneProduit.Num_Ligne = this.Fiche.LignesProduits.length + 1;
  ...
  delete(Produit_Designation) {
    let filteredLigneProduitIndex = this.Fiche.LignesProduits.findIndex(item => item.Produit.Designation == Produit_Designation);
  ...
  sauvegarder()
  {
    ...
    this.dataService.addFicheSortie(this.Fiche).subscribe(
      data => {
